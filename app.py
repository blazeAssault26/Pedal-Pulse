...
def show_predict_page():
    ...
    temperature_input = st.number_input("Temperature (°C)", min_value=-50.0, max_value=50.0, value=20.0, step=1.0, key="temperature")
    humidity_input = st.number_input("Humidity (%)", min_value=0.0, max_value=100.0, value=50.0, step=1.0, key="humidity")
    wind_speed_input = st.number_input("Wind Speed (m/s)", min_value=0.0, max_value=100.0, value=5.0, step=0.1, key="wind_speed")
    visibility_input = st.number_input("Visibility (10m)", min_value=0.0, max_value=3000.0, value=1000.0, step=100.0, key="visibility")
    solar_radiation_input = st.number_input("Solar Radiation (MJ/m2)", min_value=0.0, max_value=10.0, value=5.0, step=0.1, key="solar_radiation")
    rainfall_input = st.number_input("Rainfall (mm)", min_value=0.0, max_value=100.0, value=0.0, step=1.0, key="rainfall")
    snowfall_input = st.number_input("Snowfall (cm)", min_value=0.0, max_value=50.0, value=0.0, step=0.1, key="snowfall")
